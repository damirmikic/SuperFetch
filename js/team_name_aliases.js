/**
 * Name aliases between a competitor bookmaker's feed and the Superbet feed.
 *
 ...
 * no letters. Those go here, as `[bookName, superbetName]` tuples.
 *
 * By far the biggest group is national teams: the competitors publish them in
 * English, Superbet in Serbian, so "Turkey" has to be told it means "Turska".
 * Those are shared across books and sports; club aliases are not, because they
 * are specific to one feed's spelling and to one sport.
 *
...
/**
 * Club aliases are keyed by book and then by sport. By book because a name is
 * only ever wrong in the feed it came from, and by sport because the same short
 * name can mean different clubs in different sports - "Wolves" must not leak
 * from football into hockey. National teams above are shared by everything,
 * since a country is a country.
 */
...
export function aliasPairsFor(bookKey, sportKey) {
  return [...NATIONAL_TEAM_ALIASES, ...(CLUB_ALIASES[bookKey]?.[sportKey] ?? [])];
}
